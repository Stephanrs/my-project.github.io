(function () {
   ...
   RoomSlider.prototype.resetCss = function () {
      ...
      /*两个左右按钮公共样式*/
      createStyle.innerHTML += '.buttonCss{font-size:14px;position:absolute;z-index:2;background:rgba(222,222,222,0.4);color:#fff;padding:4px;border-style:none;outline:none;cursor:pointer;display:none;}';
      ...
      createStyle.innerHTML += '.nav{padding:0 4px;list-style:none;position:absolute;font-size:0px;background:#fff;bottom:20px;right:30px;text-align:center;-moz-border-radius:10px;border-radius:10px;}';	//nav样式
      createStyle.innerHTML += '.nav{padding:0 4px;list-style:none;position:absolute;font-size:0px;background:rgba(222,222,222,0.4);bottom:20px;right:215px;text-align:center;-moz-border-radius:10px;border-radius:10px;}';	//nav样式
      createStyle.innerHTML += '.navButton{display:inline-block;margin:6px 4px;background:#fff;width:8px;height:8px;-moz-border-radius:8px;border-radius:8px;cursor:pointer;position:relative;bottom:0px;right:0px}'	//导航点样式
   ...
   RoomSlider.prototype.resetAllButton = function () {
      ...
      this.navButton[0].style.background = '#FF0036';	//初始化第一个点的颜色
   }
      ...
      this.time = setInterval(function () {
         ...
         if (_this.imgIndex < _this.img.length) {
            ...
            if (left > (-_this.slideWidth * _this.imgIndex)) {
               ...
               _this.navButton[_this.imgIndex - 1].style.background = '';
               _this.imgIndex++;
               _this.navButton[_this.imgIndex - 1].style.background = '#FF0036';
            }
         } else {
            if (left < 0) {
   ...
   RoomSlider.prototype.navButtonStyle = function (liTarget) {
      liTarget.style.background = '#FF0036';
